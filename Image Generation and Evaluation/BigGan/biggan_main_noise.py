...
# noise scale
parser.add_argument('--noise_variance', default = 0, type=float)
parser.add_argument('--noise_layers_proc', default = 1, type = float)
...
class Biggan:
    ...
    def load_pretrained(self):
        ...
        # This code is an addition in order to apply noise the the pretrained model
        with torch.no_grad():
            for name, param in self.generator.named_parameters():
                param.add_(torch.randn(param.size()).to('cuda') * (self.config['noise_variance'])**2)
...
if __name__ == '__main__':
    ...
    config['noise_variance'] = args.noise_variance
    config['noise_layers_proc'] = args.noise_layers_proc
